...
impl TicketStoreClient {
    pub fn insert(&self, draft: TicketDraft) -> Result<TicketId, TrySendError<TicketDraft>> {
        ...
        if let Ok(_) = self.sender.try_send(Command::Insert {
            draft: draft.clone(),
    ...
    pub fn get(&self, id: TicketId) -> Result<Option<Ticket>, TrySendError<TicketId>> {
        let (sender, receiver) = std::sync::mpsc::channel();
        if let Ok(_) = self.sender.try_send(Command::Get {
            id,
            response_channel: sender,
        }) {
...
enum Command {
    ...
    Get {
        id: TicketId,
        response_channel: Sender<Ticket>,
    },
}
    ...
    loop {
        match receiver.recv() {
            ...
            Ok(Command::Get {
                ...
                let _ = response_channel.send(ticket.unwrap().clone());
            }
            Err(_) => {
